...
本项目是一个本地优先、面向全服务商务酒店负责人的个人经营治理工作台。每日经营驾驶舱负责 PMS 日报归档、客房权威指标和来源追溯；原审计与合同、人力、行政和财务能力作为管理工具箱继续保留。
...
| 领域 | 工作流 | 主要结果 |
|---|---|---|
| 酒店经营 | PMS 经营日报 | 可售房、已售房、入住率、客房收入、平均房价（ADR）和每间可售房收入（RevPAR） |
...
- CSV/XLSX 日报字段映射、原件归档、营业日版本和数据质量阻断；
- 统一案件历史和原始附件下载；
- 高、中、低风险发现及来源证据；
...
1. 在左侧填写当前操作人。
2. 默认进入“每日经营驾驶舱”，确认 PMS 来源表头并上传 CSV/XLSX 日报。
3. 查看营业日、日报版本、指标口径版本和六项客房权威指标。
4. 多行日报需映射唯一客房库存分段；合计行、重复分段和异常数字会被阻断。
5. 按需进入审计与合同、人力、行政或财务管理工具箱上传专项材料。
6. 查看结构化结果、风险、RAG 知识对照、证据和报告。
7. 在“历史与复核”记录人工结论或重新执行。
8. 在“运行监控”查看操作和模型调用，在“任务中心”跟踪行动项及最终动作审批。
...
├── hotel_imports/      # PMS 经营日报按版本保存的原始文件
...
│   ├── hotel/              # 酒店经营：日报导入、指标口径、权威计算
│   ├── ui/                 # Streamlit 页面：驾驶舱、入住引导、设置与监控
...
- RAG 同来源重复入库替换；
- Embedding 初始化不发网络请求。
- 酒店经营日报解析、指标口径验证和权威计算；
- 驾驶舱页面渲染和数据联动；
- 入住引导（Onboarding Tour）状态管理；
- 知识资料持久化、版本溯源和案件留痕；
- 运行监控和配置持久化覆盖；
...
- [酒店经理产品升级说明](docs/hotel-manager-product-upgrade.md)
- [酒店集团职能管理 RAG 工作台规划](docs/hotel-management-rag-plan.md)
- [ADR-0004：本地优先个人经营治理工作台](docs/adr/0004-local-first-personal-product.md)
